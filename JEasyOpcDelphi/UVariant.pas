...
begin
  ...
  case VarType(varin) of
   ...
   varCurrency: begin
     // not supported -> string
     msyntax := '(Ljava/lang/String;)V';
     args[0].l := JVM.StringToJString((VarToStr(varin)));
   end;
   varDate: begin
     // not supported -> string
     msyntax := '(Ljava/lang/String;)V';
  ...
  Mid := JVM.GetMethodID(Cls, '<init>', (msyntax));
  if Mid = nil
  then raise VariantInternalException.Create('Method not found - on Get correct constructor .');
...
begin
  ...
  then begin
    GetMid := JVM.GetMethodID(varinClass, UTF8String(methodID), UTF8String(output));
    if GetMid = nil
